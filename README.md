...
[![Grokmate 소개 페이지](assets/main.png)](<https://jtech-co.github.io/Grokmate/>)
...
- `/fetch` URL 요약 · `/translate` 이미지 번역(OCR) · `/repo`(=`/gh-repo`) `/gh-user` `/gh-file` GitHub 조회 · `/gl-repo` `/gl-user` `/gl-file` GitLab 조회 · `/hf-repo` `/hf-user` `/hf-file` 허깅페이스 조회 · `/hn` Hacker News 검색
...
- `/maintenance` 점검 모드 · `/health` 상태 · `/guilds` 서버별 운영 대시보드(서버 단위 일시정지·초기화·토큰 상한) · `/cost` 토큰 집계 · `/audit` 감사 로그 · `/grant-tier` `/grant-list` 구독
...
**성인(NSFW) 모드**: 연령제한 채널 + 서버 활성 + 연령 인증 + 성적 의도 **4중 게이트**를 모두 통과해야 열립니다. 3번은 **대화할 사람이 각자 한 번** `/verify-age` 를 눌러야 하며, 디스코드 자체 연령 확인과는 별개입니다(그 결과가 봇에 전달되지 않습니다). 여는 절차는 봇을 추가할 때와 `/nsfw-policy` 를 켤 때 안내가 함께 나갑니다. 자세한 건 [COMMANDS.md](COMMANDS.md#성인nsfw-모드-여는-법) 참고.

**그 외**: 자동 모더레이션(옵트인)·모더 로그 채널, 서버 합류 온보딩, 웹·X 실시간 검색(출처 표기), 24/7 운영(점검 모드·단일 인스턴스·우아한 종료·업스트림 타임아웃·**가동률 측정**).

**가동률 측정**: 봇이 살아 있는 동안 1분마다 기록을 남겨 24시간·7일·30일 가동률과 재시작 횟수를 계산합니다. 죽어 있던 시간은 아무도 기록할 수 없어 사후에 채워 넣을 수 없고, 수집 시작 이전 기간은 다운타임으로 세지 않습니다. 기록에 **이용자·서버 식별자는 들어가지 않으며** 35일 후 삭제됩니다. 현재는 운영자 명령(`/health`)에서만 보이고, 공개 상태 페이지는 데이터가 쌓인 뒤 만듭니다. 이 수치는 참고용 관측값이며 가용성 약속(SLA)이 아닙니다.
...
- **품질**: Vitest (883 tests) · ESLint (+ `eslint-plugin-boundaries` 경계 강제) · Prettier · GitHub Actions CI(불변식 회귀 게이트 포함)
