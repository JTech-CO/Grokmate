...
- **멀티모달** — 이미지·텍스트/코드 파일 분석(SFW).
- **서버 관리** — 권한 있는 사람의 자연어 지시(슬로우모드·역할·채널)와 슬래시 명령(밴·킥·타임아웃·청소, 확인 버튼).
- **모더레이션·레벨링·온보딩** — 자동 모더레이션(옵트인), XP 레벨/리더보드, 합류 시 환영.
- **게이팅된 성인 모드** — 4중 게이트(연령제한 채널 + 서버 활성 + 연령 인증 + 성적 의도)를 모두 통과해야만.

명령어 전체는 [COMMANDS.md](COMMANDS.md), 설계·보안 모델은 [ARCHITECTURE.md](ARCHITECTURE.md), 소개 페이지는 [`index.html`](<https://jtech-co.github.io/Grokmate/>).
...
Node.js 20+ (ESM) · JavaScript + JSDoc(`tsc --checkJs --strict`) · discord.js v14 · xAI Grok 4.3(`@ai-sdk/xai`) · SQLite + Drizzle ORM · undici · Vitest(301 tests) · ESLint(+ `eslint-plugin-boundaries`).
