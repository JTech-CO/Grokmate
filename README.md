...
**주요 명령어**: 대표만 추렸습니다. 전체 51개와 자세한 설명은 [COMMANDS.md](COMMANDS.md) 또는 [소개 페이지](<https://jtech-co.github.io/Grokmate/>) 참고.

_누구나_

- `/help` 안내 · `/usage` 사용량·요금제 · `/reset` 대화 초기화 · `/debug` 진단(시각·핑·시스템·네트워크 속도)
- `/fetch` URL 요약 · `/translate` 이미지 번역(OCR) · `/repo`(=`/gh-repo`) `/gh-user` `/gh-file` GitHub 조회 · `/gl-repo` `/gl-user` `/gl-file` GitLab 조회 · `/hn` Hacker News 검색
